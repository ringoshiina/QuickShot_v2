...
*   **切换等待时长 (Parcel Switch Delay)**：
    *   设置切换到下一个地块后的等待时间（毫秒），确保新页面加载完成，默认为 `3500` ms。
*   **连拍上限 (Max Capture Count)**：
    *   设置自动连拍的最大张数，防止因页面死循环或图片过多导致无限截图。
    *   设置为 `0` 表示不限制张数，默认为 `0`。
...
*   **v3.0**: 新增多地块自动切换功能，支持无人值守批量截图；优化 UI 设置，增加截图间隔和切换等待配置。
*   **v2.2**: 修复地块编号识别问题，支持多级编号（如 `F23NC011-04-03`）。
*   **v2.1**: 新增“连拍上限”设置，允许用户自定义自动连拍的最大张数。
